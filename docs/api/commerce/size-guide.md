...
```javascript
// Product has sizeGuideSlug field
const product = { sizeGuideSlug: 't-shirts-tops', ... };

// Fetch size guide by slug
const res = await fetch(`/api/v1/size-guides/slug/${product.sizeGuideSlug}`);
...
1. **Slug Management:**
   ...
   - Slugs are permanent (used in product references)
...
POST /api/v1/size-guides { name: "T-Shirts", sizes: [...] }

// 2. Store slug in product
product.sizeGuideSlug = "t-shirts"

// 3. Fetch dynamically on product page
const sizeGuide = await fetchSizeGuide(product.sizeGuideSlug);
```
